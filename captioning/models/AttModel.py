...
class AttModel(CaptionModel):
    ...
    def get_logprobs_state(self, it, fc_feats, att_feats, p_att_feats, att_masks, state, output_logsoftmax=1):
        ...
        output, state, atten = self.core(xt, fc_feats, att_feats, p_att_feats, state, att_masks)
        if output_logsoftmax:
            logprobs = F.log_softmax(self.logit(output), dim=1)
        else:
            logprobs = self.logit(output)

        return logprobs, state, atten
    ...
    def _sample_beam(self, fc_feats, att_feats, att_masks=None, opt={}):
        ...
        logprobs, state,_ = self.get_logprobs_state(it, p_fc_feats, p_att_feats, pp_att_feats, p_att_masks, state)
    ...
    def _sample(self, fc_feats, att_feats, att_masks=None, opt={}):
        ...
        for t in range(self.seq_length + 1):
            ...
            logprobs, state, att = self.get_logprobs_state(it, p_fc_feats, p_att_feats, pp_att_feats, p_att_masks, state, output_logsoftmax=output_logsoftmax)
    ...
    def _attack(self, fc_feats, att_feats, s_observed, att_masks=None, att_boxes=None, opt={} ):
        sample_method = opt.get('sample_method', 'greedy')
        ...
        output_logsoftmax = opt.get('output_logsoftmax', 1)       
        depe = opt.get('depe', 0)       
        person_cls = [1281, 2423, 4465, 5081, 8128, 8430, 7129, 9359, 1125, 6305, 3595]
        ...
        it = fc_feats.new_full([batch_size*sample_n], self.bos_idx, dtype=torch.long)     
        # loss = 0.
        loss = torch.zeros(batch_size).to(fc_feats.device)
        loss_att = torch.zeros(1).to(fc_feats.device)

        for t in range(max_step):
            id_ren = torch.zeros(batch_size,5).to(fc_feats.device)
            logi, state, atten = self.get_logprobs_state(it, p_fc_feats, p_att_feats, pp_att_feats, p_att_masks, state, output_logsoftmax=0)
            max_idx, n = torch.sort(logi, dim = 1,descending=True)
            it = torch.cat([s_observed[t].unsqueeze(0) for _ in range(batch_size)])
            if  (t == 1) | (t == 3) | (t == 6)| (t == 8) :
            # bird
            # if  (t == 1) | (t == 3) | (t == 5) | (t == 8):
            # sand    
            ...
            # if  (t == 1):
            # dog
                for nu,at in enumerate(atten):
                    for i,j in enumerate(att_boxes[nu]):
                        if(j==1):
                            loss_att += at[i]
            zheng = (s_observed[t] == n[:,0]).float()
            fu = torch.ones_like((s_observed[t] == n[:,0]).float()) - (s_observed[t] == n[:,0]).float()

            for person_cl in person_cls:
                id_ren = id_ren + (n[:,0:5]==person_cl).float()
            loss3 = (max_idx[:,0:5] * id_ren).mean(1)

            if depe == 0:
                # if(t == 1):
                #     loss1 = torch.max(max_idx[:,1]-max_idx[:,0], - 2 * torch.ones_like(max_idx[:,1])) * zheng   
                # else:
                loss1 = torch.max(max_idx[:,1]-max_idx[:,0], - 1 * torch.ones_like(max_idx[:,1])) * zheng
            elif depe == 1:
                if(t == 1):
                    loss1 = torch.max(max_idx[:,1]-max_idx[:,0], - 2 * torch.ones_like(max_idx[:,1])) * zheng   
                else:
            ...
            loss2 = (max_idx[:,0] - logi[ : , s_observed[t]]) * fu
            if depe == 0:
                loss = loss + loss1 + loss2 
            elif depe == 1:
                loss = loss + loss1 + loss2 + 0.5 * loss3  
        # for t in range(max_step):
        #     logi, state = self.get_logprobs_state(it, p_fc_feats, p_att_feats, pp_att_feats, p_att_masks, state, output_logsoftmax=0)
        #     max_idx, n = torch.sort(logi, dim = 1,descending=True)
        #     # print(max_idx,n)
        #     it = s_observed[t].view(-1)
        #     if(s_observed[t] == n[0][0]):
        #         loss = loss + max(max_idx[:,1] - max_idx[:,0] , -1+max_idx[:,1]-max_idx[:,1])
        #         # print(max(max_idx[:,1] - max_idx[:,0] , -1))
        #     else:
        #         loss = loss + max_idx[:,0] - logi[:,it]
        #         # print(max_idx[:,0] - logi[:,it])
        return loss, loss_att

    def _inference_lai_ssvm(self, fc_feats, att_feats, s_observed, slack, att_masks):
        obs_idx = torch.nonzero(s_observed).squeeze(1)
        max_idx = torch.max(obs_idx)
        ...
        p_fc_feats, p_att_feats, pp_att_feats, _ = self._prepare_feature(fc_feats, att_feats, att_masks)
        ...
        for t in range(max_step):
            ...
            output, state, _ = self.core(xt, p_fc_feats, p_att_feats, pp_att_feats, state)
            logprob = F.log_softmax(self.logit(output), dim=1)
    ...
    def _inference_gd_ssvm(self, fc_feats, att_feats, s_observed, s_hidden, att_masks):
        obs_idx = torch.nonzero(s_observed).squeeze(1)
        max_idx = torch.max(obs_idx)
        # if last obs_idx is end of caption, need add a <eos>
        if max_idx != len(s_observed) - 1 and s_observed[max_idx + 1] == 0:
...
class UpDownCore(nn.Module):
    ...
    def forward(self, xt, fc_feats, att_feats, p_att_feats, state, att_masks=None):
        ...
        att, weight = self.attention(h_att, att_feats, p_att_feats, att_masks)
...
class Att2in2Core(nn.Module):
    ...
    def forward(self, xt, fc_feats, att_feats, p_att_feats, state, att_masks=None):
        att_res, att_weight = self.attention(state[0][-1], att_feats, p_att_feats, att_masks)
